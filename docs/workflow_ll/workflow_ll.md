...
实际使用：
1）在这里调用 `/research-init "topic"`， 会**输出**: `literature-review.md` + `research-proposal.md` + `references.bib`（从 Zotero 导出）+ 带 PDF 的有序 Zotero 集合✅

2）`/zotero-review "collection"`， 这里会输出 `plan/paper-notes/..` +更新 `literature-review.md`中 `Comparison Matrix` ✅ 有一定的意义

3)这个 `/zotero-notes "collection"`我还没有测试， 有空还是可以测试下 [!todo] 📍
...
- **结构**: Hydra `_target_` 实例化 → Lightning（LightningModule + LightningDataModule + Trainer）→ 由 `rules/coding-style.md` 强制执行
  ✅ 已与 `docs/constructure_schedule.md` 合并至 `architecture-design` skill（v2.0.0），skill 为唯一架构真相源
...
│   │
│   ├── architecture-design/     # ML 项目模式：Lightning + Hydra `_target_`
...
- **类型提示**：所有函数都需要
- **实例化**：Hydra `_target_` + `hydra.utils.instantiate`
- **配置驱动**：模型 `__init__` 接受显式参数（由 Hydra 从 YAML 注入）
